...
const lukesPath = path.join(rootDir, 'lukes-case-study.html');
const visualIdentityPath = path.join(rootDir, 'visual-identity.html');

// Read current visual-identity.html which has everything intact
const visualHtml = fs.readFileSync(visualIdentityPath, 'utf8');

// Extract the Luke's case study block
const lukesStartMarker = "CASE STUDY 02 — LUKE'S ICED COFFEE";
const lukesEndMarker = "<!-- END: Luke's Iced Coffee -->";

let lukesStartIdx = visualHtml.indexOf(lukesStartMarker);

// Walk back to find the <!-- before it
if (lukesStartIdx !== -1) {
    lukesStartIdx = visualHtml.lastIndexOf('<!--', lukesStartIdx);
}

let lukesEndIdx = visualHtml.indexOf(lukesEndMarker, lukesStartIdx);

if (lukesStartIdx === -1 || lukesEndIdx === -1) {
    console.error("Could not find Luke's case study markers");
    process.exit(1);
}

// Extract block and add the end marker length
const lukesContent = visualHtml.substring(lukesStartIdx, lukesEndIdx + lukesEndMarker.length);

const otherProjectsHtml = `
    <!-- Other Projects Section -->
    <div class="w-full max-w-6xl mx-auto px-6 md:px-8 py-20 mt-12 border-t border-[#1B1717]/10 bg-zinc-100">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
        <!-- Card: Benner -->
        <a href="benner-case-study.html" class="group relative block overflow-hidden bg-zinc-900 rounded-2xl cursor-pointer" style="min-height:500px;">
...
const footerHtml = `
<!-- Footer -->
<footer class="bg-[#EDEBDD] flex flex-col md:flex-row justify-between items-center px-6 md:px-24 py-16 w-full mt-auto">
...
<head>
  ...
  <title>Luke's Iced Coffee</title>
  <meta name="description" content="Case Study for Luke's Iced Coffee by Adley Fernandes." />
...
<body id="top" class="bg-zinc-100 text-zinc-900 antialiased selection:bg-black selection:text-white flex flex-col min-h-screen">
  ...
  <header class="fixed top-0 left-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-zinc-200 shadow-sm">
    <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
      <div>
        ...
        <h1 class="text-sm md:text-base font-bold tracking-tight text-black">Luke's Iced Coffee</h1>
      </div>
  ...
  <main class="pt-28 w-full flex flex-col items-center bg-zinc-100 flex-grow">
...
fs.writeFileSync(lukesPath, htmlContent, 'utf8');
console.log('Successfully regenerated lukes-case-study.html from visual-identity.html with clean layout and case study content.');
